...
const SaoLucView: React.FC<SaoLucViewProps> = ({ currentUser, wards = ['Tân Quan', 'Tân Khai', 'Minh Đức', 'Tân Hưng'] }) => {
    ...
    const generateLTHoSoCode = (recordsList: ArchiveRecord[]) => {
        ...
        const existingSet = new Set((recordsList || []).map(r => (r.so_hieu || (r as any).code || r.data?.code || '').trim().toLowerCase()));
        let candidate = '';
        let attempts = 0;
        while (attempts < 500) {
            attempts++;
            const rand4 = Math.floor(1000 + Math.random() * 9000).toString();
            candidate = `LT-${datePrefix}-${rand4}`;
            if (!existingSet.has(candidate.toLowerCase())) break;
        }
        return candidate;
    };
                            ...
                            <tbody className="text-sm divide-y divide-gray-100">
                                {paginatedRecords.length > 0 ? paginatedRecords.map((r, idx) => (
                                    <tr key={r.id} className={`hover:bg-blue-50/50 group ${selectedIds.has(r.id) ? 'bg-blue-50' : ''}`}>
                                        ...
                                        <td className="p-3 font-mono font-bold text-blue-600 cursor-pointer hover:underline" onClick={() => setDetailRecord(r)}>{r.so_hieu}</td>
