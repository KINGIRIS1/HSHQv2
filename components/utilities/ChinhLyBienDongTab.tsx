...
const ChinhLyBienDongTab: React.FC<ChinhLyBienDongTabProps> = ({ currentUser, notify, initialRecord }) => {
    ...
    // Reset selection khi đổi tab danh sách và xóa tìm kiếm
    useEffect(() => {
        setSelectedGroups(new Set());
        setSearchTerms({ pending: '', sent: '' });
    }, [listTab]);
    ...
    const handleExportExcel = () => {
        ...
        // Helper tính getRowSpan và getGroupSTT cho sourceList
        const getSourceRowSpan = (index: number) => {
            ...
            for (let i = index + 1; i < sourceList.length; i++) {
                if (sourceList[i].data.SO_HD === current.data.SO_HD) count++;
                else break;
            }
            return count;
        };
        ...
        const getSourceGroupSTT = (index: number) => {
            ...
            if (!current.data.SO_HD) return index + 1;
            let firstIdx = index;
            while (firstIdx > 0 && sourceList[firstIdx - 1].data.SO_HD === current.data.SO_HD) {
                firstIdx--;
            }
            let groupCount = 0;
            let i = 0;
            while (i < firstIdx) {
                ...
                if (hd) {
                    while (i < firstIdx && sourceList[i].data.SO_HD === hd) i++;
                } else {
                    i++;
                }
            }
