...
const ExportModal: React.FC<ExportModalProps> = ({ isOpen, onClose, records, type, onPreview, currentView }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      if (batchOptions.length > 0) {
        setSelectedBatchKey(prev => {
          const exists = batchOptions.some(opt => opt.key === prev);
          return exists ? prev : batchOptions[0].key;
        });
      } else {
        setSelectedBatchKey('');
      }
  ...
  const generateWorkbook = (): { wb: XLSX.WorkBook, fileName: string } | null => {
    ...
    const styles = {
        ...
        nationalSlogan: { font: { name: "Times New Roman", sz: 12, bold: true }, border: { bottom: { style: "medium" } }, alignment: { horizontal: "center", vertical: "center" } },
            ...
            <div className="pt-4 flex justify-between gap-3 border-t">
                ...
                <div className="flex gap-2">
                    ...
                    <button 
                        ...
                        disabled={isExporting || batchOptions.length === 0}
                        className="flex items-center justify-center p-2 bg-white text-emerald-700 border border-emerald-300 rounded-md hover:bg-emerald-50 disabled:opacity-50 shadow-xs transition-all cursor-pointer active:scale-95"
                        title={isExporting ? 'Đang tải...' : 'Tải file Excel'}
