      ...
      const updates: Partial<RecordFile>[] = [
        {
          id: recordId,
          ...editFormData,
          ...(editFormData.completedWorkDate && !isNaN(new Date(editFormData.completedWorkDate).getTime())
            ? { completedWorkDate: new Date(editFormData.completedWorkDate).toISOString() }
            : {}),
          ...(editFormData.pendingCheckDate && !isNaN(new Date(editFormData.pendingCheckDate).getTime())
    ...
    diagnosticResults.forEach((item) => {
      item.errors.forEach((err) => {
        ...
        if (err.autoFixKey === 'deadline' && item.record.receivedDate) {
          const recD = new Date(item.record.receivedDate);
