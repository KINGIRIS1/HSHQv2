        ...
        <div className="p-4 md:p-6 overflow-y-auto flex-1 bg-slate-50/30">
            ...
            {activeTab === 'data' && (
                <div className="max-w-4xl mx-auto space-y-8">
                    {/* Hộp quản lý sao lưu dữ liệu */}
                    <div className="border border-indigo-100 rounded-[2rem] overflow-hidden bg-white shadow-xl shadow-indigo-50/50">
                        <div className="bg-gradient-to-r from-indigo-50 to-blue-50 p-6 border-b border-indigo-100 flex items-center justify-between">
                            <h3 className="text-indigo-800 font-black flex items-center gap-3 uppercase tracking-widest text-xs"> 
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="space-y-2">
                                        <label className="text-xs font-black text-slate-700 uppercase tracking-wider block">
                                            Đường dẫn thư mục lưu trữ sao lưu trên Server
                                        </label>
                                        <div className="flex gap-2">
                                            <div className="relative flex-1">
                                                ...
                                                <input
                                                    ...
                                                    placeholder="Mặc định: server/backups"
                                                    className="w-full pl-10 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium focus:bg-white focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 transition-all outline-none"
                                                />
                                            </div>
                                            <button
                                                onClick={handleSaveBackupDir}
                                                disabled={isSavingBackupDir}
                                                className="px-4 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold text-xs uppercase tracking-wider transition-all disabled:opacity-50 active:scale-95 flex items-center justify-center gap-1 shrink-0"
                                            >
                                                {isSavingBackupDir ? <Loader2 className="animate-spin" size={16} /> : <Save size={16} />}
                                                Lưu
                                            </button>
                                        </div>
                                        <p className="text-[10px] text-slate-400 font-medium">
                                            Hệ thống sẽ ghi bản sao lưu định kỳ hàng tuần trực tiếp vào thư mục chỉ định này tại Server.
                                        </p>
                                    </div>
                                </div>

                                <div className="flex flex-col justify-between p-6 border border-slate-100 rounded-2xl bg-gradient-to-b from-white to-slate-50/50">
                                    <div className="space-y-2">
                                        <h4 className="font-bold text-slate-800 text-sm">Hành động Sao lưu & Khôi phục</h4>
                                        <p className="text-xs text-slate-400 font-medium leading-relaxed">
                                            Tải xuống toàn bộ cơ sở dữ liệu hiện tại làm tệp .json hoặc tải lên một bản sao lưu trước đó để khôi phục nhanh.
                                        </p>
                                    </div>
                                    ...
                                    <div className="grid grid-cols-2 gap-3 mt-4">
                                        <button
                                            ...
                                            className="py-3 bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs uppercase tracking-widest rounded-xl transition-all shadow-md shadow-indigo-100 disabled:opacity-50 flex items-center justify-center gap-2 active:scale-95"
                                        >
                                            {isCreatingBackup ? <Loader2 className="animate-spin" size={16} /> : <Download size={16} />}
                                            {isCreatingBackup ? 'Đang chạy...' : 'Sao lưu ngay'}
                                        </button>

                                        <label className={`py-3 bg-white hover:bg-slate-50 border border-slate-200 text-slate-700 font-bold text-xs uppercase tracking-widest rounded-xl transition-all flex items-center justify-center gap-2 cursor-pointer active:scale-95 text-center ${isRestoring ? 'opacity-50 pointer-events-none' : ''}`}>
