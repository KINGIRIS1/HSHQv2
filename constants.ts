...
export const isArchiveRecordType = (type: string | null | undefined): boolean => {
  ...
  const t = type.toLowerCase().trim();
  if (t.startsWith('1.') || t.includes('sao lục') || t.includes('sao luc') || t.includes('cung cấp tài liệu') || t.includes('cung cấp dữ liệu') || t.includes('công văn')) {
    return true;
  }
...
export const getCapGiaySubStepBadgeColor = (subStep?: string | null): string => {
  if (subStep === 'tiep_nhan' || subStep === 'tiep_nhan_giao_viec') return 'bg-blue-50 text-blue-700 border-blue-200';
  if (!subStep || subStep === 'tham_dinh') return 'bg-indigo-50 text-indigo-700 border-indigo-200';
  if (subStep === 'phieu_chuyen_thue') return 'bg-amber-50 text-amber-700 border-amber-200';
  if (subStep === 'cho_tbt') return 'bg-orange-50 text-orange-700 border-orange-200';
  if (subStep === 'cho_gnt' || subStep === 'cho_nop_thue') return 'bg-yellow-50 text-yellow-700 border-yellow-200';
  if (subStep === 'in_hoan_thien' || subStep === 'hoan_thien_trinh_duyet') return 'bg-emerald-50 text-emerald-700 border-emerald-200';
  if (subStep === 'trinh_kiem_tra' || subStep === 'kiem_tra') return 'bg-purple-50 text-purple-700 border-purple-200';
  if (subStep === 'trinh_ky') return 'bg-cyan-50 text-cyan-700 border-cyan-200';
  if (subStep === 'cho_ban_giao') return 'bg-teal-50 text-teal-700 border-teal-200';
  if (subStep === 'cho_giao_1cua_tra_kq' || subStep === 'giao_mot_cua') return 'bg-green-50 text-green-700 border-green-200';
  if (subStep === 'da_tra_ket_qua' || subStep === 'chinh_ly_luu_tru' || subStep === 'da_ban_giao') return 'bg-slate-50 text-slate-700 border-slate-200';
